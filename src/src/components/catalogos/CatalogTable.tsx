...
export default function CatalogTable({ title, description, endpoint, columns }: Props) {
  ...
  function validarRequeridos(fila: Row): string | null {
    for (const c of columns) {
      if (c.required && !fila[c.key]) {
  ...
  async function handleCreate(e: React.FormEvent) {
    e.preventDefault();
    const errorValidacion = validarRequeridos(newRow);
    if (errorValidacion) {
  ...
  async function handleSaveEdit(id: string) {
    const errorValidacion = validarRequeridos(editRow);
    if (errorValidacion) {
      setError(errorValidacion);
      return;
    }
    ...
    try {
      ...
      console.log("Guardando", endpoint, id, editRow);
